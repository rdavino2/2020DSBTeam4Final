...
We can look at the distribution of the salary. 
...
We visualize some contract data.
...
We are interested in find variables to fit to our linear models. In order to find a good set of features, we do some visualizations. 

First, does age have an effect on PER? At which age is PER the largest?
...
There seem to a linear trend between 2P statisics and salary. 

How does 2P compare to 2P%? 
...
It seems 2P and 2P% have different effects on salary. We will study them in more details in the modeling part. 

We also look at the relation between 3P and salary. It appears that the effect of 3P is rather different from the effect of 2P. 
```{r}
# Scatter plot
nbadata %>%
...
# Part 3: Modeling
We want to predict the salary from stastics.  Our models appear to predict salary trend well. 
First, we look at 2P vs salary. 
